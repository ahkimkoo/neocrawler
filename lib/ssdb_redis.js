...
redis_cli.prototype.set = function(key,value,callback){console.log('set '+key);
...
redis_cli.prototype.keys = function(key,callback){
    this.ssdb_cli.request('keys',[key,'','',-1],callback);
}

redis_cli.prototype.hlist = function(key,callback){
    this.ssdb_cli.request('hlist',[key,'','',-1],callback);
}
...
redis_cli.prototype.zlist = function(key,callback){
    this.ssdb_cli.request('zlist',[key,'','',-1],callback);
}
...
redis_cli.prototype.qlist = function(key,callback){
    this.ssdb_cli.request('qlist',[key,'','',-1],callback);
}
