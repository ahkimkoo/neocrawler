...
redis_cli.prototype.hlist = function(key,callback){
    this.ssdb_cli.hlist(key.replace('*','0'),key.replace('*','z'),-1,callback);
}
...
redis_cli.prototype.qlist = function(key,callback){
    this.ssdb_cli.request('qlist',[key.replace('*','0'),key.replace('*','z'),-1],function(resp){
        if(callback){
            var err = resp[0] == 'ok'? 0 : resp[0];
            var data = [];
...
redis_cli.prototype.zlist = function(key,callback){
    this.ssdb_cli.zlist(key.replace('*','0'),key.replace('*','z'),-1,callback);
}
