...
//        res.setEncoding('utf8');
        ...
        res.on('end', function () {
            ...
            if (res_encoding == 'gzip' && typeof unzip != 'undefined') {
                unzip(bufferHelper.toBuffer(), function(err, buff) {
                    if (!err && buff) {
                        if(isbin)callback(null,res.statusCode,buff,page_encoding);
                        else callback(null,res.statusCode,iconv.decode(buff,page_encoding),page_encoding);
                    }else callback(new Error('gzip no content '+err),res.statusCode,null,page_encoding);
                });
                ...
                inflate(bufferHelper.toBuffer(), function(err, buff) {
                    ...
                    }else callback(new Error('deflate no content '+err),res.statusCode,null,page_encoding);
                });
            } else {
                if(isbin)callback(null,res.statusCode,bufferHelper.toBuffer(),page_encoding);
                else callback(null,res.statusCode,iconv.decode(bufferHelper.toBuffer(),page_encoding),page_encoding);
            }
        });
    });
    ...
    timeOuter = setTimeout(function(){
        if(req){
            ...
            callback(new Error('time out'),504,null,null);
        }
    },(timeout||30)*1000);
    ...
    req.on('error', function(e) {
        ...
        if(req){
            req = null;
            req.abort();//req.destroy();
            callback(new Error('request error'),500,null,null);
        }
    });
    req.end();
}
