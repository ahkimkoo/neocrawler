        ...
        proxyRouter.__chooseProxy(domain,request.socket.remoteAddress,request.headers,function(proxyAddr){
            ...
            proxy_request.addListener('response', function (proxy_response) {
                ...
                proxy_response.addListener('end', function() {
                    response.end();
                    logger.info(util.format('Write data to client(%s) finish, used proxy: %s, cost: %s ms, url: %s',request.socket.remoteAddress,proxyAddr,(new Date()).getTime()-startTime,request.url));
